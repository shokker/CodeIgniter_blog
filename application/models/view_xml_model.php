        ...
        function sxiToArray($sxi){
            ...
            for( $sxi->rewind(); $sxi->valid(); $sxi->next() ) {
                if($sxi->hasChildren()){
                    ...
                    array_push($this->key_array,$sxi->key());
                    array_push($this->output,form_label($sxi->key().':',implode('_',$this->key_array)));
                    array_push($this->output,form_input(implode('_',$this->key_array),set_value(implode('_',$this->key_array),strval($sxi->current())),'placeholder='.$sxi->key().' class="form-control header_form" id='.implode('_',$this->key_array)));
                    array_pop($this->key_array);
                }
            }
            return $this->output;
